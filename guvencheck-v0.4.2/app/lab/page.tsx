...
type BenchModel = "gpt-5.6-terra" | "gpt-5.6-luna";
...
export default function LabPage() {
  ...
  async function runSuite(){
    ...
    for(const test of cases){ const started=performance.now();
      try{
        const payload = test.type === "image" ? { type:"image", imageData:makeScreenshotDataUrl(test), benchmarkModel: benchModel } : { type:test.type, content:test.content, benchmarkModel: benchModel };
  ...
  return <main className="labShell">
    <header className="brandRow"><div className="logo">G</div><div><div className="brand">GüvenCheck Lab</div><div className="tagline">V0.4.2 — Terra / Luna A/B benchmark</div></div><a className="labBack" href="/">Uygulamaya dön</a></header>
    <section className="card labIntro"><h1>Benchmark</h1><p className="lead">Testler yalnızca sen başlattığında API çağrısı yapar. Ekran görüntüsü vakaları sentetiktir; gerçek kişisel veri içermez.</p>
      ...
      <div className="labModelPicker"><span>Test modeli</span><button className={benchModel==="gpt-5.6-luna"?"active":""} disabled={running} onClick={()=>{setBenchModel("gpt-5.6-luna");setResults({});}}>Luna · ucuz/hızlı</button><button className={benchModel==="gpt-5.6-terra"?"active":""} disabled={running} onClick={()=>{setBenchModel("gpt-5.6-terra");setResults({});}}>Terra · kalite referansı</button></div>
      <button className="primary" disabled={running} onClick={runSuite}>{running?"Testler çalışıyor…":`${cases.length} testi ${benchModel.endsWith("luna")?"Luna":"Terra"} ile başlat`}</button>
