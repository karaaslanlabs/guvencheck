    ...
    try {
      await fetch("/api/feedback", {
        ...
        body: JSON.stringify({
          ...
          route: analysis.meta?.route, requestId: analysis.requestId
        })
      });
  ...
  return (
    <main className="shell">
      <header className="brandRow">
        ...
        <div className="vbadge">V0.6 · Beta</div>
      </header>
      ...
      {!analysis ? (
        <section className="card heroCard">
          ...
          <p className="privacy">🔒 Ekran görüntüsü gönderilmeden önce küçültülür ve yeniden kodlanır. V0.6 kendi veritabanımızda analiz içeriği saklamaz. <a href="/privacy">Gizlilik özeti</a></p>
        </section>
      ) : (
        <section className={`card result ${analysis.level}`}>
          ...
          {analysis.meta && (
            <details className="metricsBox">
              <summary>Test ölçümü</summary>
              <div className="metricsGrid">
                <span><b>{analysis.meta.model || "—"}</b><small>Model / rota</small></span>
                <span><b>{analysis.meta.totalTokens ?? "—"}</b><small>Toplam token</small></span>
                <span><b>{analysis.meta.escalated ? "Evet" : "Hayır"}</b><small>Terra’ya yükseltildi</small></span>
                <span><b>{analysis.meta.webSearchCalls ?? 0}</b><small>Web araması</small></span>
                <span><b>{analysis.meta.latencyMs ? `${analysis.meta.latencyMs} ms` : "—"}</b><small>Gecikme</small></span>
                <span><b>{typeof analysis.meta.estimatedCostUsd === "number" ? `$${analysis.meta.estimatedCostUsd.toFixed(4)}` : "—"}</b><small>Tahmini maliyet</small></span>
              </div>
              {analysis.meta.escalated && typeof analysis.meta.firstPassScore === "number" && <p>İlk Luna skoru: <b>{analysis.meta.firstPassScore}/100</b>. Gri bölge, düşük kanıt gücü veya görsel/metindeki bağlantı nedeniyle Terra ikinci görüşü kullanıldı.</p>}
              <p>Bu maliyet tahmini tüm model çağrıları ve web arama çağrısı ücretine göre hesaplanır; faturalandırma ekranındaki tutar nihai kaynaktır.</p>
            </details>
          )}
            ...
            {feedbackState === "sent" ? (
              ...
              <>
                ...
                <div className="feedbackButtons">
                  ...
                  <button onClick={() => sendFeedback(false, "anlasilmadi")} disabled={feedbackState === "sending"}>Hayır</button>
                </div>
              </>
            )}
          </div>
      ...
      <footer>GüvenCheck · Beta V0.6 · <a href="/lab" style={{color:"inherit"}}>Lab</a></footer>
    </main>
  );
}
