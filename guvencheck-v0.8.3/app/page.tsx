...
async function createShareCard(analysis: Analysis, appUrl?: string): Promise<File | null> {
  ...
  ctx.fillText("GüvenCheck ile kontrol edildi", 88, 1245);
  ctx.fillStyle = "#9db3a8";
    ...
    <>
      ...
      {!analysis ? (
        <section className="card heroCard">
          ...
          <p className="lead">Ekran görüntüsünü yükle, mesajı yapıştır veya linki kontrol et. GüvenCheck risk işaretlerini bulsun ve ne yapman gerektiğini söylesin.</p>

          <div className="trustStrip">
            <span>Kayıt olmadan kullan</span><span>•</span><span>Gönderdiğin içerik saklanmaz</span>
          </div>
        ...
        <section className={`card result ${analysis.level}`}>
          ...
          {(analysis.actions.length > 1 || analysis.avoid.length > 0) && (
            <details className="guidanceDetails">
              <summary>Diğer önerileri göster</summary>
              <div className="guidanceBody">
                {analysis.actions.length > 1 && (
          ...
          <div className="buttonRow">
            ...
            <button className="primary share" onClick={shareResult}>{copied ? "Kopyalandı ✓" : "Ailemle paylaş"}</button>
          </div>
        </section>
      )}
      ...
      <footer>GüvenCheck · Kapalı Beta V0.8.3 · <a href="/privacy" style={{color:"inherit"}}>Gizlilik</a></footer>
    </main>
    </>
  );
}
