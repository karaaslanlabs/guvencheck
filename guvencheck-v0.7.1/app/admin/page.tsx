...
export default async function AdminPage() {
  ...
  return <main className="adminShell">
    ...
    <section className="adminGrid">
      <div className="adminMetric"><span>Anonim oturum</span><strong>{sessions.size}</strong></div>
      ...
      <div className="adminMetric"><span>Hata oranı</span><strong>{completed.length + errors.length ? `${pct(errors.length, completed.length + errors.length)}%` : "—"}</strong><small>{errors.length} hata</small></div>
      <div className="adminMetric"><span>Paylaşım</span><strong>{shares.length}</strong></div>
    ...
    <section className="adminPanel adminRecent">
      <h2>Son ürün olayları</h2>
      <div className="adminTableWrap"><table className="adminTable"><thead><tr><th>Zaman</th><th>Olay</th><th>Tür</th><th>Skor</th><th>Rota</th><th>Oturum</th></tr></thead><tbody>
        {recent.map((r,idx) => <tr key={`${r.id || idx}-${r.created_at}`}><td>{fmtDate(r.created_at)}</td><td>{r.event_type}</td><td>{r.analysis_type ? typeLabels[r.analysis_type] : "—"}</td><td>{typeof r.score === "number" ? r.score : "—"}</td><td>{r.model_route || "—"}</td><td>{r.session_id ? `${r.session_id.slice(0,8)}…` : "—"}</td></tr>)}
      </tbody></table></div>
      <p className="adminPrivacy">Bu panel analiz edilen mesajı, linki veya ekran görüntüsünü göstermez; yalnızca anonim ürün metriklerini gösterir.</p>
    </section>
  </main>;
}
