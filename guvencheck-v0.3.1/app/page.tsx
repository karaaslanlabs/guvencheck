...
const confidenceText = { low: "sınırlı", medium: "orta", high: "yüksek" } as const;
...
export default function Home() {
  ...
  const linkIsValid = tab !== "link" || value.trim().length === 0 || validUrl(value.trim());
  const canSubmit = useMemo(() => {
    if (processingImage) return false;
    if (tab === "image") return Boolean(imageData);
    if (tab === "link") return value.trim().length >= 8 && validUrl(value.trim());
  ...
  async function analyze() {
    ...
    try {
      const response = await fetch("/api/analyze", {
        ...
        body: JSON.stringify({
          type: tab,
          content: tab === "image" ? undefined : value.trim(),
  ...
  return (
    <main className="shell">
      <header className="brandRow">
        ...
        <div className="vbadge">V0.3.1</div>
      </header>
      ...
      {!analysis ? (
        <section className="card heroCard">
          ...
          {tab === "link" && !linkIsValid && <div className="hintError">http:// veya https:// ile başlayan geçerli bir link gir.</div>}
          ...
          <p className="privacy">🔒 Ekran görüntüsü gönderilmeden önce küçültülür ve yeniden kodlanır. V0.3.1 kendi veritabanımızda analiz içeriği saklamaz.</p>
        </section>
      ) : (
        <section className={`card result ${analysis.level}`}>
          ...
          {analysis.verificationStatus && analysis.verificationStatus !== "not_checked" && (
            <div className="section verifyBox">
              <div className="verifyHead">
                <h3>Harici doğrulama</h3>
                <span className={`verifyBadge ${analysis.verificationStatus}`}>{analysis.webVerified ? "Web ile doğrulandı" : "Doğrulama sonucu"}</span>
              </div>
              {analysis.verificationSummary && <p>{analysis.verificationSummary}</p>}
              {analysis.verifiedFindings && analysis.verifiedFindings.length > 0 && (
                <ul>{analysis.verifiedFindings.map((s, i) => <li key={i}><span className="listIcon">↗</span><span>{s}</span></li>)}</ul>
              )}
          ...
          <div className="modeLine">
            ...
            {analysis.mode === "ai" ? "Gerçek AI analizi" : "Demo analizi"} · Güven düzeyi: {confidenceText[analysis.confidence]}
          </div>
      ...
      <footer>GüvenCheck · Erken erişim V0.3.1</footer>
    </main>
  );
}
