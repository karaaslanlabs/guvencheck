...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      response = await fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
          ...
          store: false,
          max_output_tokens: body.type === "link" ? 1500 : 1200,
          ...(body.type === "link" ? {
    ...
    console.info("GUVENCHECK_USAGE", JSON.stringify({
      version: "0.3.1",
      type: body.type,
      model: process.env.OPENAI_MODEL || "gpt-5.6-terra",
    ...
    return NextResponse.json({ ...parsed, sources, mode: "ai", webVerified: body.type === "link" && webSearchCalls > 0 });
  } catch (error) {
