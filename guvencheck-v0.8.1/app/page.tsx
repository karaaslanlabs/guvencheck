...
async function createShareCard(analysis: Analysis): Promise<File | null> {
  ...
  ctx.fillText(shortLevelText[analysis.level].toUpperCase(), 118, 575);
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!standalone || window.sessionStorage.getItem("guvencheck_splash_seen") === "1") return;
    window.sessionStorage.setItem("guvencheck_splash_seen", "1");
    setShowSplash(true);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setLoadingPhase((v) => (v + 1) % 3), 1600);
  ...
  async function shareResult() {
    ...
    const text = `GüvenCheck sonucu: ${shortLevelText[analysis.level]} — ${analysis.score}/100. ${analysis.actions[0] || analysis.summary}`;
  ...
  return (
    <>
      ...
      <header className="brandRow">
        <div className="logo" aria-hidden="true"><img src="/icon.svg" alt="" /></div>
        <div>
      ...
      {!analysis ? (
        <section className="card heroCard">
          ...
          <p className="lead">Ekran görüntüsünü yükle, mesajı yapıştır veya linki kontrol et. GüvenCheck neye dikkat etmen gerektiğini sade Türkçeyle söylesin.</p>

          <div className="trustStrip">
            <span>Kayıt olmadan kullan</span><span>•</span><span>İçeriğin saklanmaz</span>
          </div>
          ...
          <button className="primary" disabled={!canSubmit || loading} onClick={analyze}>
            {loading ? <><span className="spinner" /> Analiz sürüyor…</> : "Kontrol et"}
          </button>

          {loading && <div className="analysisProgress" role="status" aria-live="polite">
            <span className="progressDot" />
            <div><strong>{["İçerik okunuyor", "Risk sinyalleri karşılaştırılıyor", "Gerekirse kaynaklar doğrulanıyor"][loadingPhase]}</strong><small>Bu genellikle birkaç saniye sürer.</small></div>
          </div>}

          <p className="privacy">🔒 <strong>Gönderdiğin mesaj, link veya ekran görüntüsünü kendi veritabanımızda saklamıyoruz.</strong> Görseller gönderilmeden önce küçültülür ve yeniden kodlanır. <a href="/privacy">Gizlilik özeti</a></p>
        </section>
      ) : (
        <section className={`card result ${analysis.level}`}>
          ...
          <div className="meter compactMeter" aria-hidden="true"><div style={{ width: `${analysis.score}%` }} /></div>
          ...
          <div className="section">
            ...
            {analysis.signals.length > 3 && (
              <details className="moreDetails">
                <summary>Tüm sinyalleri göster</summary>
          ...
          <div className="section actionBox">
            ...
            <ol>{analysis.actions.slice(1).map((s, i) => <li key={i}><span className="step">{i + 2}</span><span>{s}</span></li>)}</ol>
          </div>
      ...
      <footer>GüvenCheck · Kapalı Beta V0.8.1 · <a href="/privacy" style={{color:"inherit"}}>Gizlilik</a></footer>
    </main>
    </>
  );
}
