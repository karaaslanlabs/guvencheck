...
function requireBasic(request: NextRequest, expected: string | undefined, realm: string) {
  ...
  return new NextResponse(`${realm} erişimi gerekiyor.`, {
    status: 401,
...
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith("/lab")) return requireBasic(request, process.env.LAB_ACCESS_KEY, "GüvenCheck Lab");
  if (pathname.startsWith("/admin")) return requireBasic(request, process.env.ADMIN_ACCESS_KEY, "GüvenCheck Admin");
