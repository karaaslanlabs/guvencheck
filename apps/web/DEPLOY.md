...
4. Supabase production schema değişiklikleri yalnız ayrı migration gate'inde uygulanır; önce yedek/current schema doğrulanır. Mevcut `m3_2_evidence_activation` uygulanmış production için `supabase-m3-2-protection-evidence-migration.sql` delta'sı deploy öncesi ayrıca uygulanmalıdır.
...
7. Protection evidence smoke: uygun bir taahhüt vakasında `protection_candidate_eligible` → save intent/saved; kritik tarih görünür olduğunda `protection_action_due_view`; kullanıcı faydalı işaretlerse `protection_event_useful` aynı bounded request id zinciriyle doğrulanır.
8. Reuse smoke: aynı uygun link ikinci kez verildiğinde `link-reuse` route + zero current AI/web cost ve `analysis_reuse_hit` evidence doğrulanır.
9. Curated registry boşsa `curated-intelligence` hit beklenmez; sahte kayıt eklenmez.
10. Payment, Play release, broad Growth ve provider migration bu runbook ile açılmaz.
