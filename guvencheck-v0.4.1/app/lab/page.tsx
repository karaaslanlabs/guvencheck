...
export default function LabPage() {
  const [running,setRunning]=useState(false); const [results,setResults]=useState<Record<string,Result>>({}); const [suite,setSuite]=useState<Suite>("text");
  ...
  async function runSuite(){
    ...
    for(const test of cases){ const started=performance.now();
      try{
        const payload = test.type === "image" ? { type:"image", imageData:makeScreenshotDataUrl(test) } : { type:test.type, content:test.content };
  ...
  return <main className="labShell">
    <header className="brandRow"><div className="logo">G</div><div><div className="brand">GüvenCheck Lab</div><div className="tagline">V0.4.1 — zor vaka + görsel benchmark</div></div><a className="labBack" href="/">Uygulamaya dön</a></header>
