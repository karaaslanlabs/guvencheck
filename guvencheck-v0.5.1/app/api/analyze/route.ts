...
async function runOpenAI(args: {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try {
      ...
      if (!abort || attempt === 2) throw err;
      console.warn("OpenAI timeout; retrying", `attempt=${attempt}`, `model=${args.model}`, `web=${args.useWeb}`);
    } finally {
  ...
  if (!response || !response.ok) {
    console.error("OpenAI final failure", lastDetail.slice(0, 1200));
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (body.benchmarkModel) {
      ...
      const meta = {
        version: "0.5.1", model: run.model, route: "benchmark-direct", escalated: false,
    ...
    if (body.type === "link") {
      ...
      const meta = {
        version: "0.5.1", model: run.model, route: "terra-web", escalated: true, escalationReasons: ["link_web_verification"],
        inputTokens: run.usage?.input_tokens ?? null, cachedInputTokens: run.usage?.input_tokens_details?.cached_tokens ?? 0,
        outputTokens: run.usage?.output_tokens ?? null, totalTokens: run.usage?.total_tokens ?? null,
        webSearchCalls: run.webSearchCalls, latencyMs: Date.now() - startedAt, estimatedCostUsd: run.estimatedCostUsd,
        calls: [{ model: run.model, costUsd: run.estimatedCostUsd, latencyMs: run.latencyMs, webSearchCalls: run.webSearchCalls }]
    ...
    if (error instanceof Error && error.name === "AbortError") return NextResponse.json({ error: "Analiz zaman aşımına uğradı. Otomatik tekrar denememiz de sonuç vermedi.", requestId }, { status: 504 });
    if (error instanceof Error && (error.message.startsWith("OPENAI_REQUEST_FAILED") || error.message === "OPENAI_OUTPUT_MISSING")) {
      return NextResponse.json({ error: "AI analizi şu anda tamamlanamadı. Birkaç saniye sonra yeniden deneyebilirsin.", requestId }, { status: 502 });
    }
    console.error("GUVENCHECK_UNEXPECTED", requestId, error);
    return NextResponse.json({ error: "Beklenmeyen bir hata oluştu.", requestId }, { status: 500 });
  }
}
